...
def robust_insert(vals):
    ...
    med=float(statistics.median(vals)); dev=[abs(x-med) for x in vals]; mad=float(statistics.median(dev)) if dev else 0.0
    return med,max(mad,10.0),len(vals)

def source_roles(rec,length,end_window):
    roles=[]
    if not rec['rev'] and length-rec['end']<=end_window: roles.append(('+',float(length-rec['start']),'R'))
    if rec['rev'] and rec['start']<=end_window: roles.append(('-',float(rec['end']),'L'))
    return roles
...
def canonical_end_edge(s,sside,t,tside):
    aa=(s,sside); bb=(t,tside); return (aa,bb) if aa<=bb else (bb,aa)
...
def oriented_from_endpoint(seq:str,side:str,as_source:bool)->str:
...
def build_scaffolds(records,selected):
    seq={h:s for h,s in records}; nbr={}; edge_data={}
    for (aa,bb),data in selected.items(): nbr[aa]=bb; nbr[bb]=aa; edge_data[frozenset((aa,bb))]=data
    used=set(); outputs=[]; links=[]
    for cid in seq:
        if cid in used: continue
        left=(cid,'L'); right=(cid,'R')
        if left in nbr and right in nbr: continue
        free_side='L' if left not in nbr else 'R'; current_seq=seq[cid] if free_side=='L' else rc(seq[cid]); used.add(cid); outgoing=(cid,'R' if free_side=='L' else 'L')
        while outgoing in nbr:
            other=nbr[outgoing]; oid,oside=other
            if oid in used: break
            nxt=orient(seq[oid],oside); data=edge_data[frozenset((outgoing,other))]; overlap=int(data.get('overlap',0)); emitted_gap=int(data.get('emitted_gap',0))
            if data['gap']<=0:
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('contigs',type=Path); ap.add_argument('-1','--read1',type=Path,required=True); ap.add_argument('-2','--read2',type=Path,required=True); ap.add_argument('-o','--output',type=Path,required=True); ap.add_argument('--links',type=Path,required=True)
    ap.add_argument('--threads',type=int,default=2); ap.add_argument('--min-mapq',type=int,default=20); ap.add_argument('--min-support',type=int,default=3); ap.add_argument('--dominance',type=float,default=0.75); ap.add_argument('--end-window',type=int,default=500); ap.add_argument('--min-overlap',type=int,default=31); ap.add_argument('--max-gap',type=int,default=1000)
    a=ap.parse_args(); records=fasta(a.contigs); lengths={h:len(s) for h,s in records}; insert_values=[]
    for x,y in run_mapping(a.contigs,a.read1,a.read2,a.threads):
        if x['mapq']<a.min_mapq or y['mapq']<a.min_mapq or x['r']!=y['r'] or x['rev']==y['rev']: continue
        t=abs(x['tlen']) or abs(y['tlen'])
        if 100<=t<=2000: insert_values.append(t)
    insert,mad,n_insert=robust_insert(insert_values); support=defaultdict(set); gaps=defaultdict(list)
    for x,y in run_mapping(a.contigs,a.read1,a.read2,a.threads):
        ...
        for src,tgt in ((x,y),(y,x)):
            for _ori,outer,side_s in source_roles(src,lengths[src['r']],a.end_window):
                for _tori,touter,side_t in target_roles(tgt,lengths[tgt['r']],a.end_window):
                    ...
                    if -1000<=gap<=a.max_gap*2: support[edge].add(src['q']); gaps[edge].append(gap)
