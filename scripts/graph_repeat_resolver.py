...
def crossing_context(counter: Counter[tuple[str, ...]], history: list[str], extension: list[str], forward: bool) -> tuple[int, int]:
    best_support = 0
    best_len = 0
    ...
    for l_take in range(1, min(5, len(left)) + 1):
        for r_take in range(1, min(5, len(right)) + 1):
            ...
            if support > 0 and (len(key) > best_len or (len(key) == best_len and support > best_support)):
                best_support = support
                best_len = len(key)
    return best_support, best_len


def enumerate_extensions(graph: Graph, start: str, forward: bool, max_edges: int, forbidden: set[str], blocked: set[tuple[str, str]]) -> list[list[str]]:
    neighbors = graph.out if forward else graph.inc
    frontier: list[list[str]] = [[uid] for uid in neighbors[start] if uid not in forbidden and ((start, uid) if forward else (uid, start)) not in blocked]
    out: list[list[str]] = []
...
def extension_score(graph: Graph, history: list[str], ext: list[str], forward: bool, raw_ctx, proj_ctx, high_ctx, second_ctx) -> tuple[float, tuple[int, int, int, int, int, int, int, int]]:
    rs, rl = crossing_context(raw_ctx, history, ext, forward)
    ps, pl = crossing_context(proj_ctx, history, ext, forward)
    hs, hl = crossing_context(high_ctx, history, ext, forward)
    ss, sl = crossing_context(second_ctx, history, ext, forward)
    edge_score = 0.0
    chain = ([history[-1]] + ext) if forward else (ext + [history[0]])
...
def choose_lookahead(graph: Graph, history: list[str], forward: bool, forbidden: set[str], blocked: set[tuple[str, str]], raw_ctx, proj_ctx, high_ctx, second_ctx, max_edges: int, margin: float) -> list[str] | None:
    start = history[-1] if forward else history[0]
    immediate = graph.out[start] if forward else graph.inc[start]
    available = [u for u in immediate if u not in forbidden and ((start, u) if forward else (u, start)) not in blocked]
    if len(available) < 2:
        return None
    scored = []
    for ext in enumerate_extensions(graph, start, forward, max_edges, forbidden, blocked):
        score, ctx = extension_score(graph, history, ext, forward, raw_ctx, proj_ctx, high_ctx, second_ctx)
        rs, rl, ps, pl, hs, hl, ss, sl = ctx
        spanning = (rl >= 3 and rs >= 2) or (sl >= 3 and ss >= 2) or (hl >= 3 and hs >= 1) or (pl >= 3 and ps >= 1)
        if spanning:
    ...
    if best_score <= 0 or (second > 0 and best_score < second * margin):
        return None
    # Return the longest prefix with the same first edge that still has spanning context.
    same = [item for item in scored if item[1][0] == best_ext[0] and item[0] >= best_score * 0.70]
    same.sort(key=lambda item: (-len(item[1]), -item[0], item[1]))
    return same[0][1]


def edge_choice(graph: Graph, history: list[str], forward: bool, forbidden: set[str], blocked: set[tuple[str, str]], raw_ctx, proj_ctx, high_ctx, second_ctx, dominance: float, min_direct: int):
    start = history[-1] if forward else history[0]
    ...
    for uid in neighbors:
        ...
        if uid in forbidden or edge in blocked:
            continue
        score, ctx = extension_score(graph, history, [uid], forward, raw_ctx, proj_ctx, high_ctx, second_ctx)
    ...
    strong = (rl >= 3 and rs >= 2) or (sl >= 3 and ss >= 2) or (hl >= 3 and hs >= 1) or (pl >= 3 and ps >= 1)
...
def resolve(graph: Graph, raw_ctx, proj_ctx, high_ctx, second_ctx, blocked: set[tuple[str, str]], dominance: float, min_direct: int, min_length: int, lookahead_edges: int, lookahead_margin: float):
    used: set[str] = set()
    paths: list[list[str]] = []
    branch_stops = lookahead_extensions = 0
    seeds = sorted(graph.seqs, key=lambda uid: (-(graph.coverage.get(uid, 0.0) * len(graph.seqs[uid])), -len(graph.seqs[uid]), uid))
    for seed in seeds:
        ...
        for forward in (False, True):
            while True:
                ...
                ext = edge_choice(graph, path, forward, forbidden, blocked, raw_ctx, proj_ctx, high_ctx, second_ctx, dominance, min_direct)
                if ext is None and len(neighbors) > 1:
                    ext = choose_lookahead(graph, path, forward, forbidden, blocked, raw_ctx, proj_ctx, high_ctx, second_ctx, lookahead_edges, lookahead_margin)
                    if ext is not None:
        ...
        seq_len = len(graph.seqs[path[0]]) + sum(max(0, len(graph.seqs[u]) - graph.k) for u in path[1:])
        if seq_len >= min_length:
            paths.append(path)
    return paths, {"paths": len(paths), "branch_stops": branch_stops, "lookahead_extensions": lookahead_extensions, "blocked_edges": len(blocked), "claimed_orientations": len(used)}


def paths_from_branch(graph: Graph, source: str, max_edges: int, max_bp: int) -> dict[str, list[list[str]]]:
    ...
    for _ in range(max_edges):
        ...
        for path in frontier:
            node = path[-1]
            bp = len(graph.seqs[path[0]]) + sum(max(0, len(graph.seqs[u]) - graph.k) for u in path[1:])
            if bp > max_bp:
                continue
...
def simplify_bubbles(graph: Graph, raw_ctx, proj_ctx, high_ctx, second_ctx, max_edges: int, max_bp: int, margin: float) -> tuple[set[tuple[str, str]], dict[str, int]]:
    ...
    for source in graph.seqs:
        ...
        for sink, paths in by_sink.items():
            ...
            for path in paths:
                ext = path[1:]
                score, ctx = extension_score(graph, [source], ext, True, raw_ctx, proj_ctx, high_ctx, second_ctx)
            ...
            if not strong or best[0] <= 0 or best[0] < second[0] * margin:
                continue
            # Do not prune a similarly covered alternate branch; that is more likely strain variation.
            if second[1] > 0 and best[1] > 0 and second[1] > best[1] * 0.60:
                continue
            ...
            simplified += 1
            break
    return blocked, {"candidate_bubbles": bubbles, "simplified_bubbles": simplified, "blocked_edges": len(blocked)}
...
def main() -> None:
    ...
    raw_ctx, raw_stats = collect_read_contexts(graph, index, args.read1, args.read2, None, args.max_context)
    proj_ctx, proj_stats = collect_fasta_contexts(graph, index, args.projection, args.max_context)
    ...
    for high_gfa in args.highk_gfa:
        ...
        with tmp.open("w") as handle:
            for uid, seq in hg.seqs.items():
                handle.write(f">{uid}\n{seq}\n")
        ctx, st = collect_fasta_contexts(graph, index, [tmp], args.max_context, only_ambiguous=True)
        high_ctx.update(ctx)
    ...
    second_ctx, second_stats = collect_read_contexts(graph, index, args.read1, args.read2, membership, args.max_context)
    ...
    stage5, st5 = resolve(graph, raw_ctx, proj_ctx, high_ctx, second_ctx, set(), args.dominance, args.min_direct, args.min_length, args.lookahead_edges, args.lookahead_margin)
    st5.update(write_paths(stage5, graph, out / "stage5_repeat_traversal.fasta", out / "stage5_repeat_traversal.paths.tsv", args.min_length))

    blocked, simpl_stats = simplify_bubbles(graph, raw_ctx, proj_ctx, high_ctx, second_ctx, args.bubble_edges, args.bubble_bp, args.bubble_margin)
    stage6, st6 = resolve(graph, raw_ctx, proj_ctx, high_ctx, second_ctx, blocked, args.dominance, args.min_direct, args.min_length, args.lookahead_edges, args.lookahead_margin)
    st6.update(write_paths(stage6, graph, out / "stage6_graph_simplified.fasta", out / "stage6_graph_simplified.paths.tsv", args.min_length))
    ...
    (out / "repeat_resolver_stats.json").write_text(json.dumps(stats, indent=2, sort_keys=True) + "\n")
