...
def run(cmd,*,env=None,stdout=None):
    print('+',' '.join(map(str,cmd)),flush=True); st=time.monotonic(); subprocess.run(list(map(str,cmd)),check=True,env=env,stdout=stdout); return time.monotonic()-st

def bridge_cmd(exe,r1,r2,out,k,mercy,threads):
    return [exe,'assemble','-1',r1,'-2',r2,'-o',out,'-k',k,'--min-count',2,'--mercy-max-kmers',mercy,'--mercy-min-support',1,'--mercy-min-quality',25,'--min-read-support',2,'--min-pair-support',2,'--min-primary-support',5,'--primary-dominance',0.75,'--threaded-path-cover','--major-path-cover','--path-cover-secondary-dominance',0.25,'--min-contig-length',200,'--threads',threads]
...
def postprocess(scripts:Path,inputs:list[Path],outdir:Path,min_overlap=31,margin=10):
    outdir.mkdir(parents=True,exist_ok=True); merged=outdir/'union.fasta'; filtered=outdir/'noncontained.fasta'; final=outdir/'primary_contigs.fasta'
    run([sys.executable,scripts/'merge_fasta_unique.py',merged,*inputs,'--min-length',200])
    run([sys.executable,scripts/'filter_contained_fasta.py',merged,filtered,'--min-length',200,'--seed-k',21,'--window',12,'--candidate-minimizers',16,'--removed-tsv',outdir/'contained_removed.tsv','--stats-json',outdir/'containment_stats.json'])
    run([sys.executable,scripts/'stitch_exact_overlaps.py',final,filtered,'--min-overlap',min_overlap,'--overlap-margin',margin,'--seed-length',31,'--max-seed-occurrences',64,'--min-length',200]); return final
...
def main():
    ...
    iterative=out/'iterative'; iterative.mkdir(exist_ok=True); stages=[('k21_recall',21,24),('k31_resolve',31,16),('k41_resolve',41,12),('k55_resolve',55,8)]; candidates=[]; cur_r1=a.read1; cur_r2=a.read2
    for idx,(name,k,mercy) in enumerate(stages):
        ...
        candidates.append(sd/'primary_contigs.fasta')
        if idx+1<len(stages):
